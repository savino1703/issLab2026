...
*/	//USING POST : by AN Jan 2021
    app.post("/api/move", function(req, res,next)  {
		...
		try {
			...
			if( currentMove != undefined && ! currentMove.includes("collision") &&
			    ! currentMove.includes("interrupted") ){
			...
			} else{ //ESECUZIONE DEL COMANDO
	            //console.log('     $$$ WebpageServer doMove | ' + moveTodo + " duration=" + duration  );
                postResult = res  //MEMO that a POST is running. See answerToPost
...
function execMoveOnAllConnectedScenes(moveTodo, moveTime){
    ...
    //See PlayerControls.js:34 (via SocketIO)
	Object.keys(sockets).forEach( key => {  //Per tutte le socket conesse ...
         ////console.log("     $$$ WebpageServer | execMoveOnAllConnectedScenes" +  " key="+key + " sockets[key]=" + sockets[key])
	     sockets[key].emit(moveTodo, moveTime, runningMovesIndex) });
}

//Updates the controls and the observers (Jan 2021)
function updateCallers(msgJson){
//console.log("     $$$ WebpageServer | updateCallers msgJson=" + msgJson);
...
wsServer.on('connection', (ws) => {
  ...
  ws.on('message', msg => {
    ...
    //console.log("     $$$ WebpageServer wssocket=" + wssocketIndex   + " MESSAGE= "+ msg  + " curMove="+ curMove )
...
function sceneSocketInfoHandler() {
    ...
    sceneSocket.on('connection', socket => {
        ...
        socket.on( 'collision',     (obj) => {
		    ...
		    //showMoveMap(); 
		    updateCallers( JSON.stringify(info) )   //Removed April2024
        ...
        socket.on('endmove', (moveIndex)  => {  //April2022
		    ...
		    //showMoveMap( 
      		...
      		var answer   = "";
      		var moveTerminated = moveMap.get(moveIndex);
  		    ////console.log("HHH --- WebpageServer sceneSocketInfoHandler  | endmoveeeee " +
  		    //                         " curMove=" + curMove + " moveTerminated=" + moveTerminated);
  		    if( moveTerminated == undefined ) return //Feb23
  		    if( curMove == undefined ) {  //undefined viene da SocketIO.stopMoving per alarm
	              ...
	              //curMove = undefined  //GEN2023
	              //////console.log("HHH --- WebpageServer sceneSocketInfoHandler  | curMove BECOMES " + curMove + " moveIndex=" + moveIndex );
	            }
            updateCallers( JSON.stringify(answer) )  
...
function answerToPost(  answerJson ){
        ////console.log("HHH --- WebpageServer | answerToPost  answer= ' + answerJson + " postResult=" + postResult );
   if( postResult != null ){
        //////console.log("HHH --- WebpageServer | answerToPost  answer= ' + answerJson  );
