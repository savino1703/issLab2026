...
public class RobotObj26  implements IRobotBasicMoves { 
    ...
    public RobotObj26(String virtualRobotIp, IObserverMsg observer, String LogFName) {
        ...
        conn      = wsconn.getConn();
        toApplMsg = "msg(vrinfo, event, robotObj26, none, CONTENT, 0)";
    ...
    protected void handleSonar(JSONObject jsonObj) {
        if (jsonObj.get("sonarName") != null) { //defensive
            ...
            IApplMessage sonarEvent = CommUtils.buildEvent( "robotObj26","sonardata","sonar(" + d + ")");
            emitInfo(sonarEvent);
          }
    }
    ...
    protected void handleMoveko(String move) {
        ...
        if( tracing )              
        	CommUtils.outblack("robotObj26 | handleMoveKO:" + move + " elapsed=" + elapsed + " doingStepSynch " + doingStepSynch);               
    	if (move.contains("collision") || move.contains("interrupted") ) {
            if(  ! doingStepSynch ) {   
            	 move = move.split("-")[0];   //xxx-collsion
                 String wenvInfo = "msg(vrinfo, event, robotObj26, none, CONTENT, 0)"
                         .replace("CONTENT","vrinfo(" + move + "_ko ,"+ elapsed + ")");
                 IApplMessage msg = new ApplMessage(wenvInfo);   
                 if( tracing )  
                	 CommUtils.outred("robotObj26 | handleMoveKO msg:" + msg);
                 emitInfo(msg);
            } else {
            	String m = "stepFail("+elapsed+")";
                IApplMessage stepFailEvent = CommUtils.buildEvent("robotObj26","stepFail",m );
                CommUtils.outred("robotObj26 | handleMoveKO msg:" + stepFailEvent);
    ...
    protected void handleCollision( String cause) {
        ...
        IApplMessage collisionEvent = CommUtils.buildEvent(
                "robotObj26","vrinfo","vrinfo(obstacle_XXX,collision)".replace("XXX", cause) );
